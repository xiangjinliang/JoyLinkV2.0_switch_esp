...
joylink_proc_lan_sub_auth(uint8_t *src, struct sockaddr_in *sin_recv, socklen_t addrlen)
{
    ...
    /*check if gateway is at active state*/
    if(_g_pdev->jlp.is_actived){
        dev.state = E_RET_ERROR_DEV_ACTIVED;
    ...
    int en_len = 0;
    en_len = joylink_encrypt_lan_basic(_g_pdev->send_buff, 
            JL_MAX_PACKET_LEN,
...
joylink_packet_server_sub_hb_req(void)
{
    ...
    dev_heartbeat.timestamp = time(NULL);
