...
int joylink_encypt_server_xxx_rsp(
        uint8_t* pBuf, 
        int buflen, 
        E_PacketType cmd, 
        uint8_t* key, 
        ...
        int payloadLen)
{
    ...
    default:
        ...
        /*len = payloadLen;*/
		head.payloadlen = len;
        /*memcpy(pOut, payload, payloadLen);*/
		pOut += len;
        break;
    }
        ...
        char *feedid)
{
    ...
    const uint8_t * packey = (const uint8_t*)_g_pdev->jlp.accesskey;
    const uint8_t * psessionkey = (const uint8_t*)_g_pdev->jlp.sessionKey;

#ifdef _AGENT_GW_
    char ackey[JL_MAX_ACKEY_LEN];
    char sessionkey[JL_MAX_SESSION_KEY_LEN];
    if(NULL != feedid){
        if(E_RET_OK != joylink_agent_dev_get_keys(feedid, 
                      ackey,
                      sessionkey)){
            log_error("agent get keys error");
            return 0;
        }

        packey = (const uint8_t*)ackey;
        psessionkey = (const uint8_t*)sessionkey;
    }
#endif
